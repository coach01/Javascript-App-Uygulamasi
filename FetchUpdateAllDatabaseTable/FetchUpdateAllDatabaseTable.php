...
<div class="container-fluid">
  ...
  <div class="jumbotron text-center">
    ...
    <form class="form-inline" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
      ...
      <input type="text" class="form-control mb-2 mr-sm-2" id="server" placeholder="Enter server" name="server" value="<?php if(!empty($server)) echo $server; ?>">
      <label for="user" class="mb-2 mr-sm-2">User:</label>
      <input type="text" class="form-control mb-2 mr-sm-2" id="user" placeholder="Enter user" name="user" value="<?php if(!empty($user)) echo $user; ?>">
      ...
      <input type="text" class="form-control mb-2 mr-sm-2" id="database" placeholder="Enter database" name="database" value="<?php if(!empty($database)) echo $database; ?>">
      <label for="table" class="mb-2 mr-sm-2">Table:</label>
      <input type="text" class="form-control mb-2 mr-sm-2" id="table" placeholder="Enter table" name="table" value="<?php if(!empty($table)) echo $table; ?>">   
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
     echo "<tr>";
    foreach ($fieldinfo as $val) {
      echo "<td headers=\"".$val -> name."\" contenteditable onblur=\"update(this.parentElement)\">". $row[$val -> name]."</td>";
    }
    echo "</tr>";
  }
...
<div class="row">
  ...
  <div class="jumbotron text-center">
    ...
    <script>
        function lockId() {
          var length = document.querySelectorAll("td:first-child").length;
          for (var i = 0; i < length; i++) {
            document.querySelectorAll("td:first-child")[i].contentEditable = "false";
          }
        }
        function update(tableRow) {
          var query = "";
          query = "<?php echo "server=$server&user=$user&password=$password&database=$database&table=$table&" ?>";
            for (var i =  tableRow.childElementCount - 1 ; i >=0; i--) {
                query += tableRow.children[i].headers + "=";
                query += tableRow.children[i].innerText;
                if (i >= 1) {
                    query += "&";
                }

          //    document.getElementById("printArea").innerHTML = query;
            }
